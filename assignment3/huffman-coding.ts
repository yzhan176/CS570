...
//fs.writeFileSync('outfile.dat','  Final Output\n',{flag:'w'});
...
for(let j=0;j<output.length;j++){
    ...
    //fs.writeFileSync('outfile.dat','   '+temp1.value+'\t'+' '+(temp1.freq*100/Toatlfreq).toFixed(3).toString()+'%\n',{flag:'a'});
}
result+='\n';
result+='Symbol\tHuffman Codes\n';
//fs.writeFileSync('outfile.dat','\n',{flag:'a'});
//fs.writeFileSync('outfile.dat','Symbol\tHuffman Codes\n',{flag:'a'});
for(let i=0;i<output.length;i++){
    maxheap.insertHeap(output[i]);
}
for(let j=0;j<output.length;j++){
    ...
    //fs.writeFileSync('outfile.dat','   '+temp1.value+'\t'+'   '+temp1.size+'\n',{flag:'a'});
}
result+='\n';
result+='   Total Bits: '+Totalbit.toString()+'\n';
//fs.writeFileSync('outfile.dat','\n',{flag:'a'});
//fs.writeFileSync('outfile.dat',' Total Bits: '+Totalbit.toString()+'\n',{flag:'a'});
fs.writeFileSync('outfile.dat',result,{flag:'w'});

// console.log(output);
// console.log(code);
// console.log(Toatlfreq);

//console.log(maxheap.remove().freq*100/Toatlfreq);
//console.log(huffmantree.insertHuffT(minheap).right.left);
